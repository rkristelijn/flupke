...
class Negotiator {
  ...
  mediaType(available) {
    return this.mediaTypes(available)[0] || undefined;
  }
  ...
  language(available) {
    return this.languages(available)[0] || undefined;
  }
  languages(available) {
    return negotiate(
  ...
  encoding(available) {
    return this.encodings(available)[0] || undefined;
  }
  encodings(available) {
    ...
    if (header === undefined)
      return available
        ? filterAvailable(["identity"], available)
        : ["identity"];
    return negotiate(header || "identity", available, parseToken, matchToken);
  }

  charset(available) {
    return this.charsets(available)[0] || undefined;
  }
  charsets(available) {
    return negotiate(
...
function filterAvailable(specs, available, matchFn) {
  if (!matchFn)
    return available.filter((a) =>
      specs.some((s) => s === a || s.value === a || s.value === "*"),
    );
  const results = [];
