# @flupkejs/statuses

[![npm](https://img.shields.io/npm/v/@flupkejs%2Fstatuses)](https://www.npmjs.com/package/@flupkejs/statuses)
[![bundle](https://img.shields.io/bundlephobia/minzip/@flupkejs%2Fstatuses)](https://bundlephobia.com/package/@flupkejs/statuses)

Drop-in replacement for statuses

## Install

```bash
npm i @flupkejs/statuses
```

## What this replaces

Drop-in replacement for [`statuses`](https://www.npmjs.com/package/statuses). Zero dependencies, 131 lines of code.

Only the API surface used by major frameworks (Express, Next.js, Jest, etc.) is implemented — unused code is stripped.

## ⚠ Status

This package is under active development. While tested against framework usage patterns, edge cases may exist. Please [report issues](https://github.com/rkristelijn/flupke/issues) if you encounter incompatibilities.

## Part of [flupke](https://github.com/rkristelijn/flupke)

165 packages. 888 tests. Zero dependencies each.

## License

MIT
