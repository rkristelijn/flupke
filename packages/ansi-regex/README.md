# @flupkejs/ansi-regex

[![npm](https://img.shields.io/npm/v/@flupkejs%2Fansi-regex)](https://www.npmjs.com/package/@flupkejs/ansi-regex)
[![bundle](https://img.shields.io/bundlephobia/minzip/@flupkejs%2Fansi-regex)](https://bundlephobia.com/package/@flupkejs/ansi-regex)

ANSI escape regex

## Install

```bash
npm i @flupkejs/ansi-regex
```

## What this replaces

Drop-in replacement for [`ansi-regex`](https://www.npmjs.com/package/ansi-regex). Zero dependencies, 3 lines of code.

Only the API surface used by major frameworks (Express, Next.js, Jest, etc.) is implemented — unused code is stripped.

## ⚠ Status

This package is under active development. While tested against framework usage patterns, edge cases may exist. Please [report issues](https://github.com/rkristelijn/flupke/issues) if you encounter incompatibilities.

## Part of [flupke](https://github.com/rkristelijn/flupke)

165 packages. 888 tests. Zero dependencies each.

## License

MIT
