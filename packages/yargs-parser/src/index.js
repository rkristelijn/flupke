...
module.exports = function yargsParser(args) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg.startsWith('--no-')) { result[arg.slice(5)] = false; }
    else if (arg.startsWith('--')) {
      const [key, ...val] = arg.slice(2).split('=');
      result[key] = val.length ? val.join('=') : (args[i+1] && !args[i+1].startsWith('-') ? args[++i] : true);
    } else if (arg.startsWith('-') && arg.length > 1) {
      const chars = arg.slice(1);
      for (let j = 0; j < chars.length; j++) {
        if (j === chars.length - 1 && args[i+1] && !args[i+1].startsWith('-')) { result[chars[j]] = args[++i]; }
        else { result[chars[j]] = true; }
      }
