...
class IPv6 {
  ...
  toString() {
    return compressIPv6(this.parts);
  }
  toByteArray() {
    const bytes = [];
...
function expandIPv6(str) {
  ...
  if (sides.length === 2) {
    ...
    return parts.map((p) => Number.parseInt(p, 16) || 0);
  }
...
function parse(str) {
  ...
  if (v6) return v6;
  throw new Error(
    "ipaddr: the address has neither combatible IPv4 nor IPv6 format",
  );
}
...
function parseCIDR(str) {
  ...
  if (Number.isNaN(bits)) throw new Error("ipaddr: invalid CIDR prefix length");
  return [addr, bits];
}
