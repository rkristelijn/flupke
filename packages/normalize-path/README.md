# @flupkejs/normalize-path

[![npm](https://img.shields.io/npm/v/@flupkejs%2Fnormalize-path)](https://www.npmjs.com/package/@flupkejs/normalize-path)
[![bundle](https://img.shields.io/bundlephobia/minzip/@flupkejs%2Fnormalize-path)](https://bundlephobia.com/package/@flupkejs/normalize-path)



## Install

```bash
npm i @flupkejs/normalize-path
```

## What this replaces

Drop-in replacement for [`normalize-path`](https://www.npmjs.com/package/normalize-path). Zero dependencies, 3 lines of code.

Only the API surface used by major frameworks (Express, Next.js, Jest, etc.) is implemented — unused code is stripped.

## ⚠ Status

This package is under active development. While tested against framework usage patterns, edge cases may exist. Please [report issues](https://github.com/rkristelijn/flupke/issues) if you encounter incompatibilities.

## Part of [flupke](https://github.com/rkristelijn/flupke)

165 packages. 888 tests. Zero dependencies each.

## License

MIT
