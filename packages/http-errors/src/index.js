...
function toIdentifier(str) {
  return str.split(' ').map(function(p) { return p.slice(0,1).toUpperCase() + p.slice(1); }).join('').replace(/[^ _0-9a-z]/gi, '');
}
...
createError.isHttpError = function(err) {
  return err instanceof Error && typeof err.status === 'number' && err.status >= 400 && err.status < 600;
};
