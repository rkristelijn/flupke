...
describe('content-type', () => {
  describe('parse', () => {
    ...
    it('should parse content type with boundary', () => {
      const result = parse('multipart/form-data; boundary=----WebKitFormBoundary');
  ...
  describe('format', () => {
    ...
    it('should format with parameters', () => {
      const result = format({ type: 'text/html', parameters: { charset: 'utf-8' } });
      assert.strictEqual(result, 'text/html; charset=utf-8');
    });

    it('should handle special characters in value', () => {
      const result = format({ type: 'text/plain', parameters: { boundary: 'abc;def' } });
