...
test("contract: content-type.parse() exists (express/lib/utils.js:231)", () => {
  assert.equal(typeof pkg.parse, 'function', 'content-type.parse must be a function');
});

test("contract: content-type.format() exists (express/lib/utils.js:237)", () => {
  assert.equal(typeof pkg.format, 'function', 'content-type.format must be a function');
});
