...
exports.chunk = function(arr, size) { let r = []; for (let i = 0; i < arr.length; i += size) r.push(arr.slice(i, i + size)); return r; };
...
exports.uniqBy = function(arr, fn) { let seen = new Set(), f = typeof fn === 'string' ? function(o){return o[fn]} : fn; return arr.filter(function(x) { let k = f(x); if (seen.has(k)) return false; seen.add(k); return true; }); };
// Return elements in a not in b
exports.difference = function(a, b) { let s = new Set(b); return a.filter(function(x) { return !s.has(x); }); };
// Return elements present in both arrays
exports.intersection = function(a, b) { let s = new Set(b); return a.filter(function(x) { return s.has(x); }); };
exports.zip = function() { let args = Array.from(arguments); let len = Math.max.apply(null, args.map(function(a){return a.length})); let r = []; for (let i = 0; i < len; i++) r.push(args.map(function(a){return a[i]})); return r; };
...
exports.sortBy = function(arr, fn) { let f = typeof fn === 'string' ? function(o){return o[fn]} : fn; return arr.slice().sort(function(a,b) { let va = f(a), vb = f(b); return va < vb ? -1 : va > vb ? 1 : 0; }); };
// Group array elements by key function
exports.groupBy = function(arr, fn) { let f = typeof fn === 'string' ? function(o){return o[fn]} : fn; let r = {}; arr.forEach(function(x) { let k = f(x); (r[k] = r[k] || []).push(x); }); return r; };
// Index array elements by key function
exports.keyBy = function(arr, fn) { let f = typeof fn === 'string' ? function(o){return o[fn]} : fn; let r = {}; arr.forEach(function(x) { r[f(x)] = x; }); return r; };
// Find first element matching predicate or partial object
exports.find = function(arr, fn) { return arr.find(typeof fn === 'function' ? fn : function(o) { for (let k in fn) if (o[k] !== fn[k]) return false; return true; }); };
exports.findIndex = function(arr, fn) { return arr.findIndex(typeof fn === 'function' ? fn : function(o) { for (let k in fn) if (o[k] !== fn[k]) return false; return true; }); };
// Generate array of numbers from start to end
exports.range = function(start, end, step) { if (end === undefined) { end = start; start = 0; } step = step || (start < end ? 1 : -1); let r = []; if (step > 0) for (let i = start; i < end; i += step) r.push(i); else for (let i = start; i > end; i += step) r.push(i); return r; };
...
exports.get = function(obj, path, def) { let keys = Array.isArray(path) ? path : String(path).replace(/\[(\d+)]/g, '.$1').split('.'); let r = obj; for (let i = 0; i < keys.length; i++) { if (r == null) return def; r = r[keys[i]]; } return r === undefined ? def : r; };
// Set a nested property by dot-path, creating intermediates
exports.set = function(obj, path, val) { let keys = Array.isArray(path) ? path : String(path).replace(/\[(\d+)]/g, '.$1').split('.'); let o = obj; for (let i = 0; i < keys.length - 1; i++) { if (o[keys[i]] == null) o[keys[i]] = /^\d+$/.test(keys[i+1]) ? [] : {}; o = o[keys[i]]; } o[keys[keys.length-1]] = val; return obj; };
// Create object with only specified keys
exports.pick = function(obj, keys) { let r = {}; keys.forEach(function(k) { if (k in obj) r[k] = obj[k]; }); return r; };
// Create object without specified keys
exports.omit = function(obj, keys) { let s = new Set(keys); let r = {}; for (let k in obj) if (!s.has(k)) r[k] = obj[k]; return r; };
// Deep merge objects (mutates target)
exports.merge = function(target) { for (let i = 1; i < arguments.length; i++) { let src = arguments[i]; for (let k in src) { if (src[k] && typeof src[k] === 'object' && !Array.isArray(src[k])) { target[k] = exports.merge(target[k] || {}, src[k]); } else { target[k] = src[k]; } } } return target; };
...
exports.isEqual = function(a, b) { if (a === b) return true; if (a == null || b == null) return false; if (typeof a !== 'object') return false; let ka = Object.keys(a), kb = Object.keys(b); if (ka.length !== kb.length) return false; for (let i = 0; i < ka.length; i++) { if (!exports.isEqual(a[ka[i]], b[ka[i]])) return false; } return true; };
...
exports.mapValues = function(obj, fn) { let r = {}; for (let k in obj) r[k] = fn(obj[k], k); return r; };
exports.mapKeys = function(obj, fn) { let r = {}; for (let k in obj) r[fn(obj[k], k)] = obj[k]; return r; };
// Swap keys and values
exports.invert = function(obj) { let r = {}; for (let k in obj) r[obj[k]] = k; return r; };

// ═══ Functions ═══
// Utility wrappers: debounce, throttle, memoize, once
// Delay function execution until after wait ms of inactivity
exports.debounce = function(fn, ms, opts) { let t, r; return function() { let self = this, args = arguments; if (opts && opts.leading && !t) { r = fn.apply(self, args); } clearTimeout(t); t = setTimeout(function() { t = null; if (!opts || !opts.leading) r = fn.apply(self, args); }, ms); return r; }; };
// Limit function execution to once per wait ms
exports.throttle = function(fn, ms) { let last = 0; return function() { let now = Date.now(); if (now - last >= ms) { last = now; return fn.apply(this, arguments); } }; };
// Ensure function is called at most once
exports.once = function(fn) { let called = false, r; return function() { if (!called) { called = true; r = fn.apply(this, arguments); } return r; }; };
// Cache function results by first argument (or custom resolver)
exports.memoize = function(fn, resolver) { let cache = new Map(); let memoized = function() { let key = resolver ? resolver.apply(this, arguments) : arguments[0]; if (cache.has(key)) return cache.get(key); let r = fn.apply(this, arguments); cache.set(key, r); return r; }; memoized.cache = cache; return memoized; };
