# @flupkejs/is-buffer

[![npm](https://img.shields.io/npm/v/@flupkejs%2Fis-buffer)](https://www.npmjs.com/package/@flupkejs/is-buffer)
[![bundle](https://img.shields.io/bundlephobia/minzip/@flupkejs%2Fis-buffer)](https://bundlephobia.com/package/@flupkejs/is-buffer)

Drop-in replacement for is-buffer

## Install

```bash
npm i @flupkejs/is-buffer
```

## What this replaces

Drop-in replacement for [`is-buffer`](https://www.npmjs.com/package/is-buffer). Zero dependencies, 2 lines of code.

Only the API surface used by major frameworks (Express, Next.js, Jest, etc.) is implemented — unused code is stripped.

## ⚠ Status

This package is under active development. While tested against framework usage patterns, edge cases may exist. Please [report issues](https://github.com/rkristelijn/flupke/issues) if you encounter incompatibilities.

## Part of [flupke](https://github.com/rkristelijn/flupke)

165 packages. 888 tests. Zero dependencies each.

## License

MIT
